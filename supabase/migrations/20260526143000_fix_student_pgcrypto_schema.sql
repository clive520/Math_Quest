...
begin
  ...
  raw_session_token := encode(extensions.gen_random_bytes(32), 'hex');
  ...
  values (
    new_student_id,
    encode(extensions.digest(raw_session_token, 'sha256'), 'hex'),
...
begin
  ...
  where classes.class_code = upper(trim(input_class_code))
    ...
    and students.password_hash = extensions.crypt(input_password, students.password_hash)
  limit 1;
...
begin
  ...
  join public.classes
    on classes.id = students.class_id
  where student_sessions.session_token_hash = encode(extensions.digest(session_token, 'sha256'), 'hex')
